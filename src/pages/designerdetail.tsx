    ...
    <section className="xl:flex xl:min-h-0 xl:flex-col">
      ...
      >
        ...
        <div className="relative aspect-video w-full overflow-hidden bg-[#d9d9d9] xl:min-h-0 xl:flex-1 xl:aspect-auto">
          ...
          <div
            className={`pointer-events-none absolute inset-0 flex items-end bg-gradient-to-b from-transparent via-black/10 to-black/75 p-[16px] transition-opacity duration-300 group-hover:opacity-100 group-focus-visible:opacity-100 ${
              isPreviewVisible ? "opacity-100" : "opacity-0"
            }`}
          >
            <div className="flex w-full items-center justify-between gap-3">
              <p className="text-base font-medium text-white lg:text-[clamp(14px,2.2vw,16px)] lg:font-light">
                {hoverTitle}
              </p>
              <img
        ...
        <div className="grid min-h-0 flex-1 items-start gap-y-0 lg:grid-cols-[minmax(0,1.45fr)_minmax(0,1.25fr)] lg:gap-x-[clamp(28px,2.5vw,36px)] lg:gap-y-10 xl:mx-auto xl:w-[calc(100%_-_104px)] xl:max-w-[1962px] xl:grid-cols-[minmax(0,1.45fr)_clamp(28px,2.5vw,36px)_minmax(0,1.25fr)_clamp(120px,9vw,190px)_clamp(320px,21.5vw,450px)] xl:items-stretch xl:gap-0">
          <div className="flex w-full flex-col justify-end xl:col-start-1 xl:min-h-0">
            ...
            <h1 className="mb-4 flex flex-wrap items-center gap-x-2 gap-y-1 lg:hidden">
              ...
              <span className="text-[32px] font-bold text-[#000101]">{designer.name}</span>
            ...
            <div className="relative mb-[8%] w-[85%] lg:mb-0 lg:w-full">
              ...
              <div className="pointer-events-none absolute left-[90%] top-[95%] -ml-[27.5%] -mt-[27.5%] aspect-square w-[55%] lg:hidden">
                <img
                  src={`/images/object/paper/${imageNumber}.png`}
                  alt={`${designer.name} 디자이너 오브제`}
                  className="absolute inset-0 z-10 h-full w-full object-contain mix-blend-hard-light"
                />
                <img
          ...
          <section className="relative flex flex-col justify-end lg:min-h-0 lg:self-stretch xl:col-start-3">
            <div className="absolute left-[-28px] top-0 hidden h-[clamp(200px,8vw,320px)] w-[clamp(200px,8vw,320px)] lg:block xl:top-[60px]">
              <img
                src={`/images/object/paper/${imageNumber}.png`}
                alt={`${designer.name} 디자이너 오브제`}
            ...
            <div>
              <h1 className="hidden flex-wrap items-baseline gap-x-3 gap-y-1 lg:flex">
                <span className="text-[clamp(12px,2vw,40px)] font-bold text-[#000101]">{designer.name}</span>
                <span className="text-[clamp(16px,2vw,24px)] font-regular text-[#4A4B51] tracking-[-0.02em]">{getEnglishName(designer)}</span>
              </h1>
